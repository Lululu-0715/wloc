...
**Quantumult X:**
https://raw.githubusercontent.com/Lululu-0715/wloc/main/modules/wloc.conf

**Loon:**
https://raw.githubusercontent.com/Lululu-0715/wloc/main/modules/wloc.lpx
...
<details>
<summary><b>部署选点页面（Cloudflare Worker）</b></summary>

选点页面是一个纯静态网页，负责把坐标写到 `gs-loc.apple.com/wloc-settings/save`，由代理模块拦截并存入设备。

**方式一：网页版（不用命令行，手机也能做）**

1. 打开 https://dash.cloudflare.com → 左侧 **Workers & Pages** → **Create** → **Workers** → **Create Worker**
2. 起个名字（如 `wloc`）→ 点 **Deploy**
3. 点 **Edit code** → 删掉编辑器里的默认代码 → 把本仓库 `worker/wloc-worker.js` 的**全部内容**粘贴进去
4. 点右上角 **Deploy**
5. 得到地址 `https://<你起的名字>.<你的子域名>.workers.dev`，用它选点即可

> 复制 `wloc-worker.js` 内容：在 GitHub 打开该文件 → 点右上角 **Raw** → 全选复制。
...
`wrangler.jsonc` 已配置好，无需 `npm install`，也无需安装任何依赖。

**换地址后记得改模块**

如果你部署后拿到的域名不是 `wloc-1993.575613136.workers.dev`，需要把下面三个文件里的选点页面地址替换成你的新地址：

| 文件 | 字段 |
|---|---|
| `modules/wloc.sgmodule` | `#!desc` |
| `modules/wloc.conf` | `#!desc` |
| `modules/wloc.lpx` | `#!openUrl` |
